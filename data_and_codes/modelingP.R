...
##------------------------------------------------------------------
## train model for P prediction given data decimation factor of 50
##------------------------------------------------------------------
set.seed(1234)
procData <- createModelingData("P",infraredData,50)
inTrain  <- createDataPartition(y=procData$P, p=0.75, list=FALSE)

trainingP <- procData[inTrain,]
testingP  <- procData[-inTrain,]

cl <- makeCluster(3)
registerDoParallel(cl)

tic(); fitP.lm.50          <- train(P ~ ., method="lm",          data=trainingP); toc()
tic(); fitP.bagEarth.50    <- train(P ~ ., method="bagEarth",    data=trainingP); toc()
tic(); fitP.bagEarthGCV.50 <- train(P ~ ., method="bagEarthGCV", data=trainingP); toc()
tic(); fitP.earth.50       <- train(P ~ ., method="earth",       data=trainingP); toc()
tic(); fitP.gcvEarth.50    <- train(P ~ ., method="gcvEarth",    data=trainingP); toc()
tic(); fitP.rf.50          <- train(P ~ ., method="rf",          data=trainingP); toc()

stopCluster(cl)

# save models to disk
save(fitP.lm.50,          file = "saved_models/fitP.lm.50.RData")
save(fitP.bagEarth.50,    file = "saved_models/fitP.bagEarth.50.RData")
save(fitP.bagEarthGCV.50, file = "saved_models/fitP.bagEarthGCV.50.RData")
save(fitP.earth.50,       file = "saved_models/fitP.earth.50.RData")
save(fitP.gcvEarth.50,    file = "saved_models/fitP.gcvEarth.50.RData")
save(fitP.rf.50,          file = "saved_models/fitP.rf.50.RData")

##------------------------------------------------------------------
## train model for P prediction given data decimation factor of 10
##------------------------------------------------------------------
set.seed(1234)
...
tic(); fitP.lm.10          <- train(P ~ ., method="lm",          data=trainingP); toc()
tic(); fitP.bagEarth.10    <- train(P ~ ., method="bagEarth",    data=trainingP); toc()
tic(); fitP.bagEarthGCV.10 <- train(P ~ ., method="bagEarthGCV", data=trainingP); toc()
tic(); fitP.earth.10       <- train(P ~ ., method="earth",       data=trainingP); toc()
tic(); fitP.gcvEarth.10    <- train(P ~ ., method="gcvEarth",    data=trainingP); toc()
tic(); fitP.rf.10          <- train(P ~ ., method="rf",          data=trainingP); toc()

stopCluster(cl)

# save models to disk
save(fitP.lm.10,          file = "saved_models/fitP.lm.10.RData")
save(fitP.bagEarth.10,    file = "saved_models/fitP.bagEarth.10.RData")
save(fitP.bagEarthGCV.10, file = "saved_models/fitP.bagEarthGCV.10.RData")
save(fitP.earth.10,       file = "saved_models/fitP.earth.10.RData")
save(fitP.gcvEarth.10,    file = "saved_models/fitP.gcvEarth.10.RData")
save(fitP.rf.10,          file = "saved_models/fitP.rf.10.RData")

##------------------------------------------------------------------
## train model for P prediction given data decimation factor of 5
##------------------------------------------------------------------
set.seed(1234)
...
tic(); fitP.lm.5          <- train(P ~ ., method="lm",          data=trainingP); toc()
tic(); fitP.bagEarth.5    <- train(P ~ ., method="bagEarth",    data=trainingP); toc()
tic(); fitP.bagEarthGCV.5 <- train(P ~ ., method="bagEarthGCV", data=trainingP); toc()
tic(); fitP.earth.5       <- train(P ~ ., method="earth",       data=trainingP); toc()
tic(); fitP.gcvEarth.5    <- train(P ~ ., method="gcvEarth",    data=trainingP); toc()
tic(); fitP.rf.5          <- train(P ~ ., method="rf",          data=trainingP); toc()

stopCluster(cl)

# save models to disk
save(fitP.lm.5,          file = "saved_models/fitP.lm.5.RData")
save(fitP.bagEarth.5,    file = "saved_models/fitP.bagEarth.5.RData")
save(fitP.bagEarthGCV.5, file = "saved_models/fitP.bagEarthGCV.5.RData")
save(fitP.earth.5,       file = "saved_models/fitP.earth.5.RData")
save(fitP.gcvEarth.5,    file = "saved_models/fitP.gcvEarth.5.RData")
save(fitP.rf.5,          file = "saved_models/fitP.rf.5.RData")

##------------------------------------------------------------------
## train model for P prediction given data decimation factor of 1
##------------------------------------------------------------------
set.seed(1234)
procData <- createModelingData("P",infraredData,1)
inTrain  <- createDataPartition(y=procData$P, p=0.75, list=FALSE)
...
tic(); fitP.rf.1 <- train(P ~ .,method="rf",data=trainingP); toc()

stopCluster(cl)

# save models to disk
save(fitP.rf.1, file = "saved_models/fitP.rf.1.RData")
