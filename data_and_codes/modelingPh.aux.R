...
getFinalData <- function (infraredColsData,decimationFactor) {
    ...
    ## Create auxilliary-only data subset
    pat <- "EVI|TMAP|LSTD|REF7"
    auxilliaryCols <- grep(pat,names(data))
    auxilliaryData <- data.frame(data[,auxilliaryCols])
    ...
    finalData   <- cbind(data[,outcomeCols],Depth=data[,depthCol],
                         decimatedInfrared,auxilliaryData)
    finalData
}
...
tic(); fitPh.aux.lm.50          <- train(pH ~ ., method="lm",          data=trainingPh); toc()
tic(); fitPh.aux.bagEarth.50    <- train(pH ~ ., method="bagEarth",    data=trainingPh); toc()
tic(); fitPh.aux.bagEarthGCV.50 <- train(pH ~ ., method="bagEarthGCV", data=trainingPh); toc()
tic(); fitPh.aux.earth.50       <- train(pH ~ ., method="earth",       data=trainingPh); toc()
tic(); fitPh.aux.gcvEarth.50    <- train(pH ~ ., method="gcvEarth",    data=trainingPh); toc()
tic(); fitPh.aux.rf.50          <- train(pH ~ ., method="rf",          data=trainingPh); toc()

stopCluster(cl)

# save models to disk
save(fitPh.aux.lm.50,          file = "saved_models/fitPh.aux.lm.50.RData")
save(fitPh.aux.bagEarth.50,    file = "saved_models/fitPh.aux.bagEarth.50.RData")
save(fitPh.aux.bagEarthGCV.50, file = "saved_models/fitPh.aux.bagEarthGCV.50.RData")
save(fitPh.aux.earth.50,       file = "saved_models/fitPh.aux.earth.50.RData")
save(fitPh.aux.gcvEarth.50,    file = "saved_models/fitPh.aux.gcvEarth.50.RData")
save(fitPh.aux.rf.50,          file = "saved_models/fitPh.aux.rf.50.RData")
...
tic(); fitPh.aux.lm.10          <- train(pH ~ ., method="lm",          data=trainingPh); toc()
tic(); fitPh.aux.bagEarth.10    <- train(pH ~ ., method="bagEarth",    data=trainingPh); toc()
tic(); fitPh.aux.bagEarthGCV.10 <- train(pH ~ ., method="bagEarthGCV", data=trainingPh); toc()
tic(); fitPh.aux.earth.10       <- train(pH ~ ., method="earth",       data=trainingPh); toc()
tic(); fitPh.aux.gcvEarth.10    <- train(pH ~ ., method="gcvEarth",    data=trainingPh); toc()
tic(); fitPh.aux.rf.10          <- train(pH ~ ., method="rf",          data=trainingPh); toc()

stopCluster(cl)

# save models to disk
save(fitPh.aux.lm.10,          file = "saved_models/fitPh.aux.lm.10.RData")
save(fitPh.aux.bagEarth.10,    file = "saved_models/fitPh.aux.bagEarth.10.RData")
save(fitPh.aux.bagEarthGCV.10, file = "saved_models/fitPh.aux.bagEarthGCV.10.RData")
save(fitPh.aux.earth.10,       file = "saved_models/fitPh.aux.earth.10.RData")
save(fitPh.aux.gcvEarth.10,    file = "saved_models/fitPh.aux.gcvEarth.10.RData")
save(fitPh.aux.rf.10,          file = "saved_models/fitPh.aux.rf.10.RData")
...
tic(); fitPh.aux.lm.5          <- train(pH ~ ., method="lm",          data=trainingPh); toc()
tic(); fitPh.aux.bagEarth.5    <- train(pH ~ ., method="bagEarth",    data=trainingPh); toc()
tic(); fitPh.aux.bagEarthGCV.5 <- train(pH ~ ., method="bagEarthGCV", data=trainingPh); toc()
tic(); fitPh.aux.earth.5       <- train(pH ~ ., method="earth",       data=trainingPh); toc()
tic(); fitPh.aux.gcvEarth.5    <- train(pH ~ ., method="gcvEarth",    data=trainingPh); toc()
tic(); fitPh.aux.rf.5          <- train(pH ~ ., method="rf",          data=trainingPh); toc()

stopCluster(cl)

# save models to disk
save(fitPh.aux.lm.5,          file = "saved_models/fitPh.aux.lm.5.RData")
save(fitPh.aux.bagEarth.5,    file = "saved_models/fitPh.aux.bagEarth.5.RData")
save(fitPh.aux.bagEarthGCV.5, file = "saved_models/fitPh.aux.bagEarthGCV.5.RData")
save(fitPh.aux.earth.5,       file = "saved_models/fitPh.aux.earth.5.RData")
save(fitPh.aux.gcvEarth.5,    file = "saved_models/fitPh.aux.gcvEarth.5.RData")
save(fitPh.aux.rf.5,          file = "saved_models/fitPh.aux.rf.5.RData")
...
getRmsVector <- function(fit.lm,fit.bagEarth,
    ...
    decim.factor <- paste0(c("aux.",as.character(decim.factor)),collapse="")
...
errors50 <- getRmsVector(fitPh.aux.lm.50,fitPh.aux.bagEarth.50,
                         fitPh.aux.bagEarthGCV.50,fitPh.aux.earth.50,
                         fitPh.aux.gcvEarth.50,fitPh.aux.rf.50,
